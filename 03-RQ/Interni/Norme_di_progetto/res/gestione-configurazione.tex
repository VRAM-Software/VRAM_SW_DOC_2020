\subsection{PRC-Q4 Gestione della configurazione}
	...
	\subsubsection{Attività}
		...
		\newline
		\newline
		Le modifiche effettuate ai documenti ed ai file contenenti codice sorgente sono identificate da un numero di versione, comune per l'interno prodotto, presente all'interno dei file stessi. Per quanto riguarda i documenti, il numero di versione possiede sempre una corrispondente riga nella tabella delle modifiche, presente nei file stessi, che descrive i cambiamenti avvenuti all'interno dei singoli file. I salti di versione in queste tabelle, dovuti all'aggiornamento degli altri sottoprodotti, saranno indicati in modo sintetico nella tabella stessa. Il numero di versione viene inoltre inserito, ad ogni rilascio del prodotto, nel nome dei documenti.
		\newline
		Per il codice sorgente del software, le modifiche effettuate in una versione sono consultabili direttamente sulla sezione rilasci di GitHub e saranno identificate da opportuni tag.
		\newline
		\newline
		La numerazione per il versionamento\glosp del prodotto si base sulle modifiche applicate al repository\glosp principale grafana\_prediction, che contiene al suo interno gli altri repository\glosp come sottomoduli. Ad ogni modifica in esso effettuata, indipendentemente dal sottomodulo che la causa, il numero di versione del prodotto viene incrementato secondo lo schema seguente.
		
		\subparagraph*{Numerazione delle versioni del prodotto}
		Per identificare univocamente le versioni del prodotto utilizziamo una numerazione basata su 3 cifre nel formato X.Y.Z, la prima bozza ha versione 0.0.0 e le modifiche al codice aggiorneranno le cifre della versione nel modo seguente:
		\begin{itemize}
