\section{Processi primari}
	...
	\subsection{Sviluppo}
		...
		\end{itemize}
		\subparagraph{Metriche di qualità} %INSERIRE QUALITA DEI REQUISITI VEDI RA 8LAB
		Le metriche di qualità utilizzate per valutare la qualità dell'analisi dei requisiti sono:
		\begin{itemize}
			\item \textbf{percentuale di requisiti obbligatori soddisfatti (PROS)}: indica la percentuale di requisiti obbligatori che sono stati soddisfatti. Questo valore deve essere massimizzato per garantire che i vincoli col proponente vengano rispettati;
			\begin{itemize}
				\item[] \textbf{formula}: $\frac{requisiti \; obbligatori \; soddisfatti}{requisiti \; obbligatori \; totali}$;
			\end{itemize} 
			\item \textbf{percentuale di requisiti desiderabili soddisfatti (PRDS)}:
...
\begin{itemize}
	\item \textbf{JavaScript}: piattaforma sulla quale si intende sviluppare il plug-in e il programma di addestramento esterno utilizzando in questo caso l'estensione JSX che permette di gestire elementi React (libreria JavaScript);
	\item \textbf{HTML}: linguaggio di mark-up che nell'ambito del progetto\glosp viene principalmente generato dall'estensione JSX e gestisce il front-end dell'applicazione;
	\item \textbf{CSS}: linguaggio usato per formattare in modo corretto documenti HTML. Nell'ambito di questo progetto\glosp viene utilizzato per impartire regole stilistiche al codice generato per gestire il front-end e la rappresentazione degli elementi grafici.
\end{itemize}
Tutti i linguaggi elencati devono rispettare le regole indicate nel seguente paragrafo quando esse possono essere applicate.
...
\end{lstlisting}
	...
	\item \textbf{Indentazione}: i blocchi annidati devono essere indentati con una tabulazione di quattro spazi. A tal fine le IDE di tutti i componenti del gruppo dovranno essere configurate per rispettare suddette indicazioni. Segue un esempio di indentazione corretta:
\begin{lstlisting}
// OK!
function() {
var x = 1;
...
\end{lstlisting}
	\item \textbf{Blocchi if-else}: ogni blocco if deve avere un corrispondente blocco else che verrà posizionato in linea con la parentesi graffa di chiusura dell'if precendente. Se i blocchi sono di una singola riga la parentesi graffa può essere omessa. Segue un esempio della corretta applicazione di questa pratiche:
\begin{lstlisting}
// OK!
if (condition) {
//TODO
...
\end{itemize}
\subparagraph*{SonarJS e linee guida}\mbox{}\\ [1mm]
Per quanto concerne i restanti aspetti riguardanti lo stile e le best practises sul codice vengono prese a riferimento dal gruppo le seguenti fonti:
\begin{itemize}
	\item \textbf{SonarJS}: strumento di analisi statica del codice che, oltre alla segnalazione di bug, indica anche delle linee guida per una migliore fruizione del codice. Tale obiettivo viene raggiunto anche grazie all'utilizzo di SonarLint, uno strumento atto all'analisi del codice per segnalare eventuali incongruenze con le best practises (\url{https://www.sonarsource.com/products/codeanalyzers/sonarjs.html});
	\item \textbf{Linee guida di Grafana}\glo: vengono utilizzate anche delle linee guida per la programmazione in React da parte di Grafana\glo. Tali indicazioni si rifanno principalmente alla guida Airbnb React/JSX Style Guide. Di seguito il link ad entrambe le risorse:
	\begin{itemize}
		\item \url{https://github.com/grafana/grafana/blob/master/contribute/style-guides/frontend.md};
		\item \url{https://github.com/airbnb/javascript/tree/master/react}.
	\end{itemize}
	\item \textbf{W3C}: per quanto riguarda l'HTML e il CSS invece il gruppo si atterrà alle regole imposte dal World Wide Web Consortium. Per assicurarsi il raggiungimento di tali norme il codice prodotto verrà verificato tramite i verificatori offerti da suddetto consorzio. Di seguito il link ai validatori utilizzati:
	\begin{itemize}
...
\end{itemize}
		...
		\begin{itemize}
			\item \textbf{livello di annidamento}: indica il livello di annidamento all'interno dei metodi. Se questo valore è alto il codice è più complesso e difficile da manutenere. Perciò è opportuno suddividere il metodo in più metodi distinti per ottenere un livello di annidamento basso;
			\begin{itemize}
				\item[] \textbf{formula}: si calcola con un valore intero;
			\end{itemize}
			\item \textbf{numero di parametri per metodo}: indica il numero di parametri presenti in ciascun metodo. Se il valore è elevato è molto probabile che il grado di complessità del metodo sia elevato. Perciò è opportuno limitare questo numero ed eventualmente suddividere il metodo in più metodi distinti;
			\begin{itemize}
				\item[] \textbf{formula}: si calcola con un valore intero.
			\end{itemize}
		\end{itemize}
