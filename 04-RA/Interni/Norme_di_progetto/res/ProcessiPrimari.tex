\section{Processi primari}
	\subsection{PRC-Q1 Fornitura}
		...
		\subsubsection{Attività}
			...
			\paragraph{Pianificazione}\mbox{}\\ [1mm]
				...
				Per lo svolgimento di questa attività sono necessari due documenti:
				\begin{itemize}
					\item \textit{Piano di Qualifica};
					\item \textit{Piano di Progetto}.
				\end{itemize}
				\paragraph*{Piano di Qualifica}\mbox{}\\ [1mm]
				Questo documento è redatto dai verificatori per la parte retrospettiva e dai progettisti per la parte programmatica. Questo documento si pone l'obiettivo di garantire la qualità di prodotto\glosp e la qualità di processo\glo. Esso è composto da:
				\begin{itemize}
				...
				Questo documento rappresenta concretamente la pianificazione del lavoro da svolgere è un documento redatto dal responsabile, in collaborazione con gli amministratori. Esso è composto da:
				\begin{itemize}
	...
	\subsection{PRC-Q2 Sviluppo}
		...
		\end{itemize}
			\textbf{Classificazione}: definisce l'importanza che, nonostante sia già presente nel codice identificativo, aumenta la facilità di lettura e di comprensione.\\
			\\ \textbf{Descrizione}: fornisce una spiegazione concisa, ma completa, del requisito.\\
			\\ \textbf{Fonti}: possono essere:
			\begin{itemize}
		...
		\paragraph{Progettazione}
			...
			\end{itemize}
			\paragraph*{Proof of Concept}\mbox{}\\ [1mm]
			...
			\paragraph*{Diagramma delle classi}\mbox{}\\ [1mm]
	...
	\subsubsection{Obiettivi di qualità}
		...
		\subparagraph{Metriche}
			\begin{itemize}
				...
				\begin{itemize}
					\item[] \textbf{formula}: si calcola con un valore intero.
				\end{itemize}
				\item \textbf{M-PROC20 livello di annidamento}: indica il livello di annidamento all'interno dei metodi. Se questo valore è alto, il codice è più complesso e difficile da manutenere. Perciò è opportuno suddividere il metodo in più metodi distinti per ottenere un livello di annidamento basso;
				\begin{itemize}
					\item[] \textbf{formula}: si calcola con un valore intero.
				\end{itemize}
			\end{itemize}
		\paragraph{OP-9 Forte disaccoppiamento tra le componenti architetturali}
		...
		\subparagraph{Metriche}
		\begin{itemize}
			\item \textbf{M-PROC21 Profondità della gerarchia}: indica la profondità massima della gerarchia delle classi nel caso peggiore. Deve essere limitata per mantenere un basso accoppiamento;
			\begin{itemize}
				\item[] \textbf{Formula}: si calcola con un valore intero.
			\end{itemize} 	
			\item \textbf{M-PROC22 Numero di design pattern}: indica il numero di design pattern applicati. La presenza di design pattern può aiutare all'innalzamento della qualità del prodotto ma un uso eccessivo può comportare un'elevata complessità. Va quindi mantenuto un numero di design pattern ragionevole;
			\begin{itemize}
				\item[] \textbf{Formula}: si calcola con un valore intero.
			\end{itemize} 	
		\end{itemize}
		...
		\paragraph{CP-3 Completezza e adeguatezza dei requisiti}
		Questa caratteristica definisce l'obiettivo di individuare correttamente i requisiti nella prima attività di analisi in modo che con l'avanzare del progetto non ci siano cambiamenti che comportano un grosso dispendio di tempo.
		\subparagraph{Metriche}
			\begin{itemize}
